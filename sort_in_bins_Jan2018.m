...
ls_all_T=polyval(p_all_T,[min(wet),1, max(wet)]);
...
ls_md_T=polyval(p_md_T,[min(wet),1, max(wet)]);

[r2, p2]=corrcoef(yW, d_md_W, 'rows', 'pairwise');
r2_2=r2(1,2)^2;
p2=p2(1,2);

Good22 = ~(isnan(yW) | isnan(d_mn_W)) ;
p_mn_T=polyfit(yW(Good22), d_mn_W(Good22), 1);
ls_mn_T=polyval(p_mn_T,[min(wet),1, max(wet)]);


%Wind
Good3 = ~(isnan(ff) | isnan(dichte95)) ;
p_all_W=polyfit(ff(Good3), dichte95(Good3),1);
ls_all_W=polyval(p_all_W,[min(ff),0.5, max(ff)]);
...
ls_md_W=polyval(p_md_W,[min(ff),0.5, max(ff)]);

[r4, p4]=corrcoef(yW1, d_md_W1, 'rows', 'pairwise');
r2_4=r4(1,2)^2;
p4=p4(1,2);

Good44 = ~(isnan(yW1) | isnan(d_mn_W1)) ;
p_mn_W=polyfit(yW1(Good44), d_mn_W(Good44), 1);
ls_mn_W=polyval(p_mn_W,[min(ff),0.5, max(ff)]);
...
subplot(1,2,1)
 ...
 hold on
 plot([min(wet),1, max(wet)],ls_all_T, 'color','b', 'lineWidth', 2, 'LineStyle','-.');
 hold on
 plot([min(wet),1, max(wet)],ls_mn_T, 'color','r', 'lineWidth', 2, 'LineStyle','-.');
 hold on
 plot([min(wet),1, max(wet)],ls_md_T, 'color','k', 'lineWidth', 2, 'LineStyle','-.');
 
 
legend({'All', 'Mean', 'Median', 'ls All', 'ls Mean', 'ls Median'}, 'Location','southeast', 'FontSize',14,'FontWeight','bold');
...
subplot(1,2,2)
 ...
 hold on
 plot([min(ff),0.5, max(ff)],ls_all_W, 'color','b', 'lineWidth', 2, 'LineStyle','-.');
 hold on
 plot([min(ff),0.5, max(ff)],ls_mn_W, 'color','r', 'lineWidth', 2, 'LineStyle','-.');
 hold on
 plot([min(ff),0.5, max(ff)],ls_md_W, 'color','k', 'lineWidth', 2, 'LineStyle','-.');
